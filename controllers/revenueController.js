...
export const getFameScoreDashboard = async (req, res) => {
  try {
    const talents = await Talent.find({ fame_score: { $ne: null } })
      .select(
        "name symbol tier status fame_score qualified qualification_reason fame_score_breakdown estimated_monetization_value total_shares next_re_evaluation_at createdAt"
      )
      .sort({ fame_score: -1 })
      .lean();
    ...
    const rows = talents.map((t) => ({
      ...
      estimated_monetization_value: t.estimated_monetization_value
        ? parseFloat(t.estimated_monetization_value.toString())
        : null,
